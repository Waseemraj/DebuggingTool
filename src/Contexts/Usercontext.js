...
export const Usermapping = (props)=>{
    ...
    const USER_DETAIL_API =`${BASE_URL}/identity/scim/8845b4ec-5fde-49df-8837-53932da40be2/v1/Users/${User.userdetail}`
    let msuserid;
    const checkID  = async (items,User) =>  {
              console.log(items)
              console.log('Email validation ', /\S+@\S+\.\S+/.test(User.userdetail))
              if(true){
                await axios.get(USER_DETAIL_API, { headers: {"Authorization" : `Bearer ${User.token}`} })
                .then(res => {
