...
export const Forms = () => {
    ...
    async function handleSubmit(e) {
        ...
        if (User.environment === 'Production') {
            ...
            // If the user wasn't found in any of the objects, show "not exist" alert
            if (!userExists) {
            ...
            const api = `https://msft-presence-service.int-first-general1.ciscospark.com/msft-presence-service/api/v1/allUserMapping`
            const intsub_api = `https://msft-presence-service.int-first-general1.ciscospark.com/msft-presence-service/api/v1/allSubscriptions`


            await axios.get(api, { headers: { "Authorization": `Bearer ${User.token}` } })
                .then(res => { allmaps = res.data; setinfo(true); checkID(res.data, User) })

            await axios.get(intsub_api, { headers: { "Authorization": `Bearer ${User.token}` } })
